...
| Field | Value |
|-------|-------|
| **Branch** | `reserver-industrial` |
| **Hostname** | `mediabox` |
| **Platform** | `aarch64-linux` |
| **Role** | NVIDIA Jetson Orin NX (media / edge) |
| **Status** | Active |

Notable configuration on this branch:

- Hostname **`mediabox`** (differs from the branch name)
- `nvidia-jetpack` with `som = "orin-nx"`, `super = true`, CUDA configured
- NVIDIA Container Toolkit enabled; Docker rootless **disabled**
- OpenSSH on port **36122** (key-only)
- jetpack-nixos via `vendor/jetpack-nixos` submodule
- Closely related to `reserver-industrial-16gb` (same hostname; see that branch for deltas)
...
Layout on this branch (`reserver-industrial`) — includes `vendor/`:
...
- This machine: `aarch64-linux` (Orin NX, super mode)

### Services (this machine)

- jetpack / CUDA, nvidia-container-toolkit
