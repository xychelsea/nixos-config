...
| Field | Value |
|-------|-------|
| **Branch** | `jetson-agx-orin` |
| **Hostname** | `llmbox` |
| **Platform** | `aarch64-linux` |
| **Role** | NVIDIA Jetson Orin AGX (LLM / CUDA workloads) |
| **Status** | Active |

Notable configuration on this branch:

- Hostname **`llmbox`** (differs from the branch name)
- `nvidia-jetpack` with `som = "orin-agx"`, CUDA configured
- NVIDIA Container Toolkit enabled; Docker rootless **disabled**
- OpenSSH on port **36122** (key-only); firewall allows that port
- jetpack-nixos via `vendor/jetpack-nixos` submodule (`.gitmodules`)
- Minimal Home Manager (shell modules); `allowUnfree = true`

Checkout path on this host: `/persist/etc/nixos`. Initialize submodules before rebuilds.
...
Layout on this branch (`jetson-agx-orin`) — includes `vendor/`:
...
- **Home Manager:** as pinned in `configuration.nix` / jetpack integration
- **`install.sh` channel URLs:** still 25.11 (see Installation note)

### Architecture

- This machine: `aarch64-linux` (Orin AGX)

### Services (this machine)

- jetpack / CUDA stack, nvidia-container-toolkit
- Docker (persist data-root; rootless off)
- OpenSSH on **36122**

### Boot / vendor

- jetpack-nixos modules from `./vendor/jetpack-nixos`
- Shared Linux impermanence + LUKS/Btrfs patterns where applicable on this image
