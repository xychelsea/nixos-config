...
- Headless-oriented: no Hyprland/SDDM desktop session
- OpenSSH on port **36122** with password authentication disabled
- Firewall allows TCP **36122**
- Kernel pinned to `linuxPackages_6_12`
- Docker with data-root under `/persist`; minimal Home Manager (shell modules only)
...
- OpenSSH (`:36122`, key-only)
- Docker (persist data-root)
- NetworkManager / networking as configured — not a Hyprland desktop host

### Boot (this machine)

- GRUB (EFI) with theme under `grub-theme/`
- LUKS2 + Btrfs subvolumes + tmpfs `/home`
- `linuxPackages_6_12`
